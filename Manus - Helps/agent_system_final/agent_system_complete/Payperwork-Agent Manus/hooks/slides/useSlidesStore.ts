...
interface SlidesStore {
  ...
  updateAgentStatus: (agent: string, status: string, currentAction?: string, progress?: number) => void;
...
export const useSlidesStore = create<SlidesStore>()((set, get) => ({
  ...
  clearAgentStatus: () => {
    slidesLogger.debug('Clearing agent status', {
      action: 'clearAgentStatus',
    });
    set({ agentStatus: {} });
  },
  ...
  clearPipelineProgress: () => {
    slidesLogger.debug('Clearing pipeline progress', {
      action: 'clearPipelineProgress',
    });
    set({
      pipelineProgress: {
        completedPhases: [],
