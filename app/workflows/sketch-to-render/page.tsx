...
import { useSketchToRender } from "@/hooks/workflows/useSketchToRender";
...
export default function SketchToRenderPage() {
  ...
  const [renderSettings, setRenderSettings] = useState<RenderSettingsType>(DEFAULT_RENDER_SETTINGS);
  ...
  useEffect(() => {
    const loadGenerations = async () => {
      try {
        ...
        const response = await fetch(`/api/sketch-to-render/generations?userId=${encodeURIComponent(userId)}`);
        if (response.ok) {
